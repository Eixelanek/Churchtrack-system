...
try {
    ...
    if (!empty($data->username) && !empty($data->password)) {
        ...
        $query = $hasRoleColumn
            ? "SELECT id, username, password, role FROM admin WHERE username = :username LIMIT 1"
            : "SELECT id, username, password FROM admin WHERE username = :username LIMIT 1";
        ...
        if ($stmt->rowCount() > 0) {
            ...
            if ($hasRoleColumn && isset($row['role']) && $row['role'] === 'manager') {
            ...
            try {
                ...
                $updateHistory->bindParam(':admin_id', $id, PDO::PARAM_INT);
                ...
                $insertHistory->bindParam(':admin_id', $id, PDO::PARAM_INT);
                ...
                $deactivateSessions->bindParam(':admin_id', $id, PDO::PARAM_INT);
                ...
                $insertSession->bindParam(':admin_id', $id, PDO::PARAM_INT);
