...
const Manager = () => {
  ...
  const renderEvents = () => {
    const [eventFormType, setEventFormType] = React.useState('preset');
    ...
    const PRESETS = [
      { key: 'sunday-service', name: 'Sunday Service', time: '09:00', end: '12:00' },
      { key: 'prayer-meeting', name: 'Prayer Meeting', time: '18:00', end: '20:00' },
    ];
    ...
    const handlePresetClick = (preset) => {
      setEventFormType('preset');
      setSelectedService(preset.key);
    ...
    const handleCreateEvent = async () => {
      const title = eventFormType === 'preset'
        ? (PRESETS.find(p => p.key === selectedService)?.name || '')
        : customName.trim();
      ...
      try {
        const res = await fetch(`${backendBaseUrl}/api/events/create.php`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            title,
        ...
        <div className="qr-form-panel" style={{ maxWidth: 520 }}>
          ...
          <div className="qr-form-card">
            ...
            <div className="qr-form-tabs">
              <button
                type="button"
                className={`qr-tab ${eventFormType === 'preset' ? 'active' : ''}`}
                onClick={() => { setEventFormType('preset'); setCustomName(''); }}
              >
                Preset Service
              </button>
              <button
                type="button"
                className={`qr-tab ${eventFormType === 'custom' ? 'active' : ''}`}
                onClick={() => { setEventFormType('custom'); setSelectedService(''); }}
              >
                Custom Event
              </button>
            </div>
