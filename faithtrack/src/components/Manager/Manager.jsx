...
const Manager = () => {
    ...
    const fetchDashboardStats = async () => {
      ...
      try {
        ...
        setDashboardStats((prev) => ({
          ...
          totalEventsThisMonth: Number(stats.totalEventsThisMonth) || 0
        }));
      } catch (error) {
  ...
  useEffect(() => {
    const fetchUpcomingEvents = async () => {
      ...
      try {
        const response = await fetch(`${backendBaseUrl}/api/dashboard/get_upcoming_events.php?limit=5`);
        const responseText = await response.text();
        const result = responseText ? JSON.parse(responseText) : null;

        if (!response.ok || !result?.success || !Array.isArray(result.data)) {
          throw new Error(result?.message || 'Failed to load upcoming events');
        }

        setUpcomingEventsRaw(result.data);
      } catch (error) {
  ...
  const dashboardSummary = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: placeholder ?? formatNumber(dashboardStats.totalEventsThisMonth),
        meta: placeholder ? 'Loading...' : 'total events',
        icon: 'calendar'
      }
    ];
    ...
    <div className="mgr-dashboard">
      ...
      <div className="mgr-grid">
        ...
        <div className="mgr-col-left">
          ...
          <section className="mgr-card">
            <div className="mgr-card-head">
              <span className="mgr-card-title">Upcoming Events</span>
              <button type="button" className="mgr-view-all" onClick={() => setActiveView('events')}>View all</button>
            </div>
            {isUpcomingEventsLoading ? (
              <div className="mgr-empty">Loading…</div>
            ) : upcomingEventsRaw.length === 0 ? (
              <div className="mgr-empty">No upcoming events</div>
            ) : (
              <div className="mgr-records-list">
                {upcomingEventsRaw.map((event) => {
                  ...
                  return (
                    <div key={event.id} className="mgr-record-row">
                      <div className="mgr-record-dot" />
                      <div className="mgr-record-body">
                        <span className="mgr-record-title">{event.title}</span>
                        <span className="mgr-record-date">{dateLabel} · {event.time}</span>
                      </div>
                      {event.location && (
                        <span className="mgr-record-count" style={{ fontSize: '0.72rem', color: 'var(--text-muted, #888)', whiteSpace: 'nowrap', maxWidth: '90px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{event.location}</span>
                      )}
                    </div>
                  );
                })}
      ...
      <header className="manager-topbar">
        ...
        {showMobileMenu && (
          <div className="mobile-menu-dropdown">
            {navItems.map((item) => {
              const icons = {
                'dashboard': <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>,
                'scan-attendance': <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/></svg>,
                'events': <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
                'attendance': <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="8.5" cy="7" r="4"/><polyline points="17 11 19 13 23 9"/></svg>,
                'members': <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
              };
              return (
                <button
                  key={item.key}
                  className={`mobile-menu-item ${activeView === item.key ? 'active' : ''}`}
                  onClick={() => { setActiveView(item.key); setShowMobileMenu(false); }}
                  type="button"
                >
                  {icons[item.key] || null}
                  {item.label}
